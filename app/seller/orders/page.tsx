...
function OrderCard({ order, labelBusy, labelError, onGenerateLabel, onUpdateStatus, onAddTracking }: OrderCardProps) {
  // Per-card state lives INSIDE the child component — never call hooks in a
  // .map() callback (that crashes React's hook ordering).
          ...
          <div className="mt-2">
            {order.label_url ? (
              ...
              <div className="space-y-2">
                ...
                >
                  <Truck className="h-4 w-4" />
                  {labelBusy ? 'Generating label...' : 'Generate shipping label'}
                </button>
                {labelError && (
                    ...
                    <button
                      onClick={() => {
                        if (trackingInput.trim()) {
                          onAddTracking(order.id, trackingInput.trim())
